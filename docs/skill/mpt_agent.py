...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install B-roll and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"B-roll installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional B-roll CLI arguments placed after --",
    )
...
def ensure_project(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        if len(candidates) != 1:
            raise SkillError("download completed but no valid B-roll project was found")
        if root.exists():
            root.rmdir()
...
def result_manifest_path(root: Path) -> Path:
    return root / ".agent-logs" / AGENT_LOG_DIR_NAME / "latest-result.json"
...
def generate_video(
    ...
    log_dir = root / ".agent-logs" / AGENT_LOG_DIR_NAME
